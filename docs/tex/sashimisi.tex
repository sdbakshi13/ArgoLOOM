...
\begin{document}
	...
	\subsection*{Ultra-Quick Deployment Checklist}
	
	For users who want the integration running immediately, follow this minimal task list.
	
	\begin{tcolorbox}[title=Fastest Working Deployment Path]
		
		\textbf{Step 1 — Activate environment}
		\begin{lstlisting}
			conda activate sashimi-si
		\end{lstlisting}
		
		\textbf{Step 2 — Ensure dependencies installed}
		\begin{lstlisting}
			pip install openai sentence-transformers
			conda install -c conda-forge faiss-cpu -y
		\end{lstlisting}
		
		\textbf{Step 3 — Copy integration code}
		
		\begin{itemize}
			\item Replace your \texttt{argo-loom.py} with the updated version from:
			
			\url{https://github.com/sdbakshi13/ArgoLOOM/tree/main}
			
			\item Copy the directory:
		...
		\textbf{Step 4 — Edit absolute paths}
		...
		\textbf{Step 5 — Set OpenAI API key}
		
		\begin{lstlisting}
			export OPENAI_API_KEY=sk-...
		\end{lstlisting}
		
		\textbf{Step 6 — Launch ArgoLOOM}
	...
	\begin{itemize}
		\item The tool \texttt{sashimi\_subhalos} will be called.
		\item SASHIMI progress will execute.
		\item A catalog and summary JSON will be produced.
	\end{itemize}
	
	
	---
	
	\section{Following sections have the details of the implementation}
